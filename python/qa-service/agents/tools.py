...
def _api_write(method: str, path: str, body: dict | None = None, timeout: int = 30) -> dict:
    """向 admin-service 发送 PUT/DELETE 等写请求并保留服务端错误正文。"""
    url = f"{ADMIN_SERVICE_URL}/api/admin/{path}"
    ...
    try:
        ...
        try:
            ...
            pass
        logger.error("API %s %s failed: HTTP %s", method, path, exc.code)
...
def fetch_evaluation_skills(database_id: str = "") -> dict:
    """从管理数据库读取评估 Skills。"""
    suffix = f"?databaseId={quote(database_id)}" if database_id else ""
    return _api_get(f"evaluation/skills{suffix}")


def fetch_evaluation_skill(skill_id: str) -> dict:
    return _api_get(f"evaluation/skills/{quote(skill_id)}")


def create_evaluation_skill(skill: dict) -> dict:
    return _api_post("evaluation/skills", skill, timeout=30)


def update_evaluation_skill(skill_id: str, skill: dict) -> dict:
    return _api_write("PUT", f"evaluation/skills/{quote(skill_id)}", skill, timeout=30)


def delete_evaluation_skill(skill_id: str) -> dict:
    return _api_write("DELETE", f"evaluation/skills/{quote(skill_id)}", timeout=30)
...
def execute_sql_on_dataset(dataset_id: str, sql: str) -> dict:
    """在数据集绑定的数据源上执行只读 SQL，并触发 Java 端数据集范围二次校验。"""
    return _api_post(f"dataset/{dataset_id}/execute-sql", {"sql": sql}, timeout=120)
