...
import json
import logging
from .crewdefs import CHART_AGENT

logger = logging.getLogger("evaluation.chart")

CHART_SYSTEM_PROMPT = f"""# 角色: {CHART_AGENT['role']}
# 目标: {CHART_AGENT['goal']}

{CHART_AGENT['backstory']}

---

你是图表设计专家。根据 SQL 查询结果的列名和样本数据，设计最合适的 ECharts 图表。
...
def _parse_chart_response(text: str) -> dict:
    ...
    处理 LLM 可能输出 markdown 代码块包裹的情况，并做容错解析：
    先从 ```json 或 ``` 标记中提取内容，再尝试 JSON.parse，
    失败时用正则兜底匹配。
    ...
    Returns:
        dict: 规范的图表配置，包含 vizType、chartTitle、xAxis、yAxis
    """
    text = text.strip()

    # 提取 JSON：处理 LLM 可能返回 markdown 代码块的情况
    if "```json" in text:
