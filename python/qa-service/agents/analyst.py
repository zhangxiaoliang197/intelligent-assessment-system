...
## ⚠️ 第一步（最重要！）：判断数据是否与问题相关
请先对比"用户问题"和"数据预览"的内容：
- 如果数据主题（如表名、字段名、数值含义）与用户问题的领域完全无关 → **数据不相关**
- 对比数据源的内容领域与用户问题的领域，不一致时不要强行关联

**如果数据不相关**，直接忽略该数据，用你自己的知识回答用户问题，并在回答开头简短说明："查询到的数据库内容与当前问题不相关，以下为基于知识的分析："

## 第二步（仅在数据相关时）：基于数据给出建议
- 只输出2-3条建议，每条1-2句话
- 建议要具体、可操作
- 引用数据中的具体数值
- 不要输出长篇报告
