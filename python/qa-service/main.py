...
from llm_transport import LlmTransportError, post_chat_json, stream_chat_lines
...
_default_cors_origins = "http://localhost:10086,http://127.0.0.1:10086"
_cors_origins = [
    origin.strip()
...
def load_llm_config():
    """通过内部鉴权接口获取当前活跃配置（含完整 API Key）。"""
    internal_token = os.getenv("ADMIN_INTERNAL_TOKEN", "").strip()
    if not internal_token:
        logger.error("ADMIN_INTERNAL_TOKEN is not configured; refusing to fetch the LLM secret")
    ...
    try:
        ...
        logger.warning(f"Failed to fetch internal LLM config from admin-service: {e}")
...
def call_llm_api(query, context=""):
    ...
    try:
        data = post_chat_json(url, api_key, body, timeout=120)
...
def stream_llm_api(query, context="") -> Generator[str, None, tuple]:
    ...
    try:
        for line in stream_chat_lines(url, api_key, body, timeout=120):
            data_str = line[6:]
            if data_str == "[DONE]":
                break
            try:
...
async def get_llm_config():
    """获取浏览器可见的活跃配置；admin-service 会对 API Key 做掩码。"""
    try:
        req = urllib.request.Request(
        ...
        return {"success": False, "message": "无法连接到管理服务", "data": _default_llm_config()}
