...
def _http(method: str, path: str, body: dict = None, timeout: int = None) -> dict:
...
def save_report(report: dict) -> dict:
    ...
    upd = _http("PUT", f"/api/admin/situation/reports/{report_id}", report)
    if upd.get("success"):
        return upd
    return _http("POST", "/api/admin/situation/reports", report)


def get_report(report_id: str) -> dict:
    return _http("GET", f"/api/admin/situation/reports/{report_id}")


def list_reports(user_id: str = "", team_ids: list = None, page: int = 1, size: int = 20) -> dict:
    params = f"page={page}&size={size}"
    if user_id:
        params += f"&userId={user_id}"
    if team_ids:
        params += f"&teamIds={','.join(team_ids)}"
    return _http("GET", f"/api/admin/situation/reports?{params}")


def delete_report(report_id: str) -> dict:
    return _http("DELETE", f"/api/admin/situation/reports/{report_id}")


def create_share(report_id: str) -> dict:
    return _http("POST", f"/api/admin/situation/reports/{report_id}/share")
