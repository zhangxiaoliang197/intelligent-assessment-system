...
def get_skill_markdown(skill_id: str, user_id: str, *, is_admin: bool = False) -> Dict[str, Any]:
    ...
    if skill.get("isBuiltIn"):
        ...
        editable = skill.get("ownerId") == user_id and skill.get("status") != "archived"
    return {
        "skillId": skill_id,
    ...
    *,
    ...
    with _WRITE_LOCK:
        ...
        if current_document["source"] == "builtin":
            if not is_admin:
                raise SkillCatalogError("只有管理员可以编辑内置 Skill Markdown")
