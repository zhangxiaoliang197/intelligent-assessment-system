...
-- 2.7 评估分析 Skills（步骤按有序 JSON 保存）
CREATE TABLE IF NOT EXISTS `ass_evaluation_skill` (
    `id`          VARCHAR(32)   NOT NULL,
    `name`        VARCHAR(100)  NOT NULL,
    `description` TEXT          DEFAULT NULL,
    ...
    PRIMARY KEY (`id`),
    UNIQUE KEY `uk_eval_skill_database_name` (`database_id`, `name`),
    INDEX `idx_eval_skill_database` (`database_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
SELECT '  表: ass_database_config, ass_dataset, ass_field_annotation, ass_indicator, ass_llm_config, ass_evaluation_skill' AS '';
