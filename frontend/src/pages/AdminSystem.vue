...
async function saveDatabase() {
  ...
  try {
    if (editingDbId.value) {
      const payload: any = { ...dbForm.value }
      // 编辑时密码留空表示保留原密码，避免把已保存的密码覆盖为空字符串。
      if (!payload.password) delete payload.password
      const res = await api.put(`/admin/database/${editingDbId.value}`, payload)
      if (res && res.success) {
        // 编辑后状态重置为未连接
...
const apiKeyPlaceholderVal = computed(() => {
  if (llmForm.value.type === 'vllm') return '本地部署无需密钥'
  if (editingLlmId.value && editingApiKeyConfigured.value) return '已配置，留空将保留原密钥'
  return '请输入API Key'
})
...
function openLlmDialog(row?: any) {
  if (row) {
    ...
    editingApiKeyConfigured.value = Boolean(row.apiKeyConfigured || row.apiKey === '********')
    llmForm.value = {
      name: row.name || '', type: row.type || 'deepseek', apiUrl: row.apiUrl || '',
      // 服务端不会把真实密钥发给浏览器；编辑时留空，由后端保留原值。
      apiKey: '', model: row.model || '',
